<template>
    <div class="wrapper">
        ...
        <div class="status-header">
            ...
            <ModeIndicator
                v-else-if="isEdit"
                :title="t('demo', 'Editing recipe')"
            />
            <ModeIndicator
                v-else-if="isRecipe"
                :title="t('demo', 'Viewing recipe')"
            />
            <!-- INDEX PAGE -->
            <Location v-if="isIndex" :title="t('demo', 'All recipes')" />
            <Location
            ...
            <Location
                v-else-if="isLoading"
                :title="t('demo', 'Loading app')"
            />
            <!-- Is a recipe loading? -->
            <Location
                v-else-if="isLoadingRecipe"
                :title="t('demo', 'Loading recipe')"
            />
            <!-- No recipe found -->
            <Location
                v-else-if="recipeNotFound"
                :title="t('demo', 'Recipe not found')"
            />
            <!-- No page found -->
            <Location
                v-else-if="pageNotFound"
                :title="t('demo', 'Page not found')"
            />
            <!-- Create new recipe -->
            <Location
                v-else-if="isCreate"
                :title="t('demo', 'Creating new recipe')"
            />
        </div>
        {{/* Primary buttons */}}
        <NcButton
        ...
        >
            <NcActionButton
                ...
                "
                class="action-button"
                :aria-label="t('demo', 'Reload recipe')"
                @click="reloadRecipeEdit()"
            >
                {{ t("demo", "Reload recipe") }}
            </NcActionButton>
            <NcActionButton
                v-if="isEdit"
                class="action-button"
                :aria-label="t('demo', 'Abort editing')"
                @click="goToRecipe($store.state.recipe.id)"
            >
                {{ t("demo", "Abort editing") }}
                <template #icon>
                    <NcLoadingIcon
                        v-if="
            ...
            <NcActionButton
                v-if="isRecipe"
                class="action-button"
                :aria-label="t('demo', 'Print recipe')"
                @click="printRecipe()"
            >
            ...
            <NcActionButton
                ...
                icon="icon-delete"
                class="action-button"
                :aria-label="t('demo', 'Delete recipe')"
                @click="deleteRecipe()"
            >
                {{ t("demo", "Delete recipe") }}
            </NcActionButton>
        </NcActions>
    </div>
</template>
    ...
    computed: {
        ...
        searchTitle() {
            if (this.$route.name === "search-category") {
                return t("demo", "Category")
            }
            if (this.$route.name === "search-name") {
                return t("demo", "Recipe name")
            }
            if (this.$route.name === "search-tags") {
                return t("demo", "Tags")
            }
            return t("demo", "Search for recipes")
        },
    },
    methods: {
        async deleteRecipe() {
            ...
            if (
                !(await showSimpleConfirmModal(
                    // prettier-ignore
                    t("demo", "Are you sure you want to delete this recipe?")
                ))
            ) {
                return
            }
            ...
            try {
                ...
                await showSimpleAlertModal(t("demo", "Delete failed"))
