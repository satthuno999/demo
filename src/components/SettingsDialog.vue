<template>
    ...
    >
                ...
                <ul>
                    <li>
                        <NcButton @click="reindex">
                            ...
                            {{ t("demo", "Rescan library") }}
                        </NcButton>
                    </li>
                    <li>
                        <label class="settings-input">{{
                            t("demo", "Recipe folder")
                        }}</label>
                        <input
                            type="text"
                            :value="recipeFolder"
                            :placeholder="t('demo', 'Please pick a folder')"
                            @click="pickRecipeFolder"
                        />
                    </li>
                    <li>
                        <label class="settings-input">
                            {{ t("demo", "Update interval in minutes") }}
                        </label>
                        <input
        ...
        <NcAppSettingsSection
            ...
            :title="t('demo', 'Recipe display settings')"
            class="app-settings-section"
        >
            <fieldset>
                <ul>
                    <li>
                        ...
                        <label for="recipe-print-image">
                            {{ t("demo", "Print image with recipe") }}
                        </label>
                    </li>
                    <li>
                        <input
                        ...
                        <label for="tag-cloud">
                            {{
                                // prettier-ignore
                                t("demo", "Show keyword cloud in recipe lists")
                            }}
                        </label>
                    </li>
                </ul>
    ...
    watch: {
        async printImage(newVal, oldVal) {
            ...
            try {
                ...
                await showSimpleAlertModal(
                    // prettier-ignore
                    t("demo","Could not set preference for image printing")
                )
        ...
        async updateInterval(newVal, oldVal) {
            ...
            try {
                ...
                await showSimpleAlertModal(
                    // prettier-ignore
                    t("demo","Could not set recipe update interval to {interval}",
                        {
                            interval: newVal,
                        }
                    )
        ...
        async visibleInfoBlocks(newVal, oldVal) {
            ...
            try {
                ...
                await showSimpleAlertModal(
                    t("demo", "Could not save visible info blocks")
                )
    ...
    methods: {
        ...
        pickRecipeFolder() {
            OC.dialogs.filepicker(
                t("demo", "Path to your recipe collection"),
                (path) => {
                    const $this = this
                    this.$store
                        ...
                        .catch(() =>
                            showSimpleAlertModal(
                                // prettier-ignore
                                t("demo","Could not set recipe folder to {path}",
                                {
                                    path
                                }
                            )
