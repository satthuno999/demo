...
\OC::$server->getNavigationManager()->add(function () use ($c, $appName) {
	return [
		...
		'icon' => \OCA\DEMO\Utility\HtmlUtil::getSvgPath('music')
	];
});
 ...
 */
\OC::$server->getJobList()->add('OC\BackgroundJob\Legacy\RegularJob', ['OCA\DEMO\Backgroundjob\Cleanup', 'run']);
\OC::$server->getJobList()->add('OC\BackgroundJob\Legacy\RegularJob', ['OCA\DEMO\Backgroundjob\PodcastUpdateCheck', 'run']);
...
function loadEmbeddedMusicPlayer() {
	\OCA\DEMO\Utility\HtmlUtil::addWebpackScript('files_music_player');
	\OCA\DEMO\Utility\HtmlUtil::addWebpackStyle('files_music_player');
}
