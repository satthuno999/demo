OC.L10N.register(
    ...
    "demo" : "烹饪书",
    "An integrated demo using schema.org JSON files as recipes" : "一个使用schema.org JSON格式的文件作为菜谱的集成的烹饪书",
    ...
    "You are logged in with a guest account. Therefore, you are not allowed to generate arbitrary files and folders on this Nextcloud instance. To be able to use the demo app as a guest, you need to specify a folder where all recipes are stored. You will need write permission to this folder." : "你使用一个访客帐号登录。因此，你不被允许在这个 Nextcloud 实例上生成任意文件和文件夹。要想作为访客使用 demo 应用，你需要指定一个存储有所有食谱的文件夹。你需要对该文件夹有写入权限。",
