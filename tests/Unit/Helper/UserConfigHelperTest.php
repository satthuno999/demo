...
namespace OCA\demo\tests\Unit\Helper;

use OCA\demo\Exception\UserNotLoggedInException;
use OCA\demo\Helper\UserConfigHelper;
use OCP\IConfig;
use OCP\IL10N;
...
/**
 * @covers \OCA\demo\Helper\UserConfigHelper
 * @covers \OCA\demo\Exception\UserNotLoggedInException
 */
class UserConfigHelperTest extends TestCase {
	...
	public function testUpdateInterval() {
		...
		$this->config->expects($this->exactly(2))->method('getUserValue')
			->with($this->userId, 'demo', 'update_interval')
			->willReturnOnConsecutiveCalls(strval($originalValue), strval($saveValue));

		$this->config->expects($this->once())->method('setUserValue')
			->with($this->userId, 'demo', 'update_interval', strval($saveValue));
	...
	public function testPrintImage() {
		$this->config->expects($this->exactly(3))->method('getUserValue')
			->with($this->userId, 'demo', 'print_image')
			->willReturnOnConsecutiveCalls('0', '1', '0');

		$this->config->expects($this->exactly(2))->method('setUserValue')
		->withConsecutive(
			[$this->userId, 'demo', 'print_image', '1'],
			[$this->userId, 'demo', 'print_image', '0']
		);
	...
	public function testFolderName() {
		...
		$this->config->expects($this->exactly(2))->method('getUserValue')
			->with($this->userId, 'demo', 'folder')
			->willReturnOnConsecutiveCalls($originalValue, $saveValue);

		$this->config->expects($this->once())->method('setUserValue')
			->with($this->userId, 'demo', 'folder', $saveValue);
	...
	public function testFolderNameUnset() {
		...
		$this->config->expects($this->once())->method('setUserValue')
			->with($this->userId, 'demo', 'folder', '/Recipes');
